...
type Node interface {
	Index() int
	Parent() Node
	Satisfy(map[string]struct{}) bool
	Threshold() int
	Equal(Node) bool
...
func (l *leafNode) Satisfy(attrs map[string]struct{}) bool {
	_, ok := attrs[string(l.Attr)]
	return ok
}
...
func (n *nonLeafNode) Satisfy(attrs map[string]struct{}) bool {
	result := false
